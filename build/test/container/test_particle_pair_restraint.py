...
class PairContainerTest(IMP.test.TestCase):
    ...
    def test_srestraint(self):
        ...
        r = self.create_pair_restraint(m, gs, p)
        r.set_was_used(True)
        self.assertAlmostEqual(r.evaluate(False), f, delta=.1 * f)
    ...
    def test_min_restraint(self):
        ...
        for i in range(0, 10):
            c.add(self.create_particle_pair(m))
    ...
    def test_set(self):
        ...
        cs = []
        for i in range(0, 3):
            lcc = IMP.container.ListPairContainer(m)
            c.add_pair_container(lcc)
            for j in range(0, 3):
                t = self.create_particle_pair(m)
                lcc.add(t)
                cs.append(t)
        ret = c.get_contents()
        if IMP.IMP_KERNEL_HAS_NUMPY and "particle_pair" != "particle":
