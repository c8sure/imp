...
# from tools/build/container_templates/test.py
...
class SingletonContainerTest(IMP.test.TestCase):
    ...
    def test_srestraint(self):
        ...
        r = self.create_singleton_restraint(m, gs, p)
        r.set_was_used(True)
        self.assertAlmostEqual(r.evaluate(False), f, delta=.1 * f)
    ...
    def test_set(self):
        ...
        cs = []
        for i in range(0, 3):
            lcc = IMP.container.ListSingletonContainer(m)
            c.add_singleton_container(lcc)
            for j in range(0, 3):
                t = self.create_particle(m)
                lcc.add(t)
                cs.append(t)
        ret = c.get_contents()
        if IMP.IMP_KERNEL_HAS_NUMPY and "particle" != "particle":
